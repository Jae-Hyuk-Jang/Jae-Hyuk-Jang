<!--### 👋 안녕하세요, 장재혁입니다.-->

<!--
**Jae-Hyuk-Jang/Jae-Hyuk-Jang** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->

<!--[![Gmail](https://img.shields.io/badge/Gmail-lucentejjh%40gmail.com-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:lucentejjh@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-Jae--Hyuk--Jang-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Jae-Hyuk-Jang) -->

<!--![slice](https://capsule-render.vercel.app/api?type=slice&color=auto&height=200&text=Hi%20there👋&fontAlign=70&rotate=13&fontAlignY=25&desc=jaeHyuk's%20GitHub&descAlign=70.&descAlignY=44)-->


## 👋 About Me!

<div align="center">


</div>

- 🌱 배운 것을 공유하고 함께 고치며 성장하는 오픈소스의 방식으로, 사람들에게 필요한 서비스를 만들고 싶습니다.
- 🧩 웹 풀스택, AI, LLM을 함께 다루며, 기능 구현을 넘어 안정화와 품질 표준화까지 책임지는 개발을 지향합니다.
- ⚙️ 작게 만들고 빠르게 검증하며, 바꾸기 쉽고 확장 가능한 구조를 선호합니다.

<div align="center">

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![C](https://img.shields.io/badge/C-A8B9CC?style=flat-square&logo=c&logoColor=black)
![C++](https://img.shields.io/badge/C%2B%2B-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=flat-square&logo=kotlin&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white)
![NestJS](https://img.shields.io/badge/NestJS-E0234E?style=flat-square&logo=nestjs&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-6DB33F?style=flat-square&logo=springboot&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![TypeORM](https://img.shields.io/badge/TypeORM-FE0902?style=flat-square&logo=typeorm&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![MariaDB](https://img.shields.io/badge/MariaDB-003545?style=flat-square&logo=mariadb&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-FF6F00?style=flat-square)
![LightGBM](https://img.shields.io/badge/LightGBM-02569B?style=flat-square)
![CatBoost](https://img.shields.io/badge/CatBoost-FFCC00?style=flat-square&logoColor=black)
![Naver Cloud](https://img.shields.io/badge/Naver%20Cloud-03C75A?style=flat-square)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white)
![Turbo](https://img.shields.io/badge/Turborepo-EF4444?style=flat-square&logo=turborepo&logoColor=white)
![pnpm](https://img.shields.io/badge/pnpm-F69220?style=flat-square&logo=pnpm&logoColor=white)

</div>

**🌱 I’m currently learning**
- 연합우주(fediverse)와 ActivityPub 프로토콜
- 🤔 AI 시대에서 문제 정의의 중요성을 체감하는 중

**🎓 Education**

- **2020.03 ~ 2026.08** : 가천대학교 · 컴퓨터공학부(컴퓨터공학전공)  
  전공 4.3/4.5 · 전체 4.28/4.5

**💫  Experience**

- 2026.07 ~ `now`   · 오픈소스 컨트리뷰션 아카데미 (Fedify)
- 2026.04 ~ 2026.08 · H-모빌리티클래스 자율주행 Track (통신 및 네트워크) 기본 + 심화 과정
- 2026.04 ~ 2026.06 · 오픈소스 컨트리뷰션 아카데미 (Yocto Project) 
- 2025.08 ~ 2026.02 · 네이버 웹·모바일 부스트캠프 10기 멤버십
- 2026.01 ~ 2026.02 · LG Aimers 8기
- 2025.07 ~ 2025.08 · 네이버 웹·모바일 부스트캠프 10기 챌린지
- 2025.07 ~ 2025.08 · LG Aimers 7기
- 2025.06 ~ 2025.08 · SK AI Dream Camp 고급
- 2025.07 ~ 2025.08 · 삼성전자 DX 부문 하계 SW 역량강화 특강
- 2025.02 ~ 2025.02 · 삼성전자 DX 부문 동계 대학생 S/W 알고리즘 특강

**🏆 Selected Achievements**

- **2026.01** : K리그-서울시립대 공개 AI 경진대회 **금상** _(K리그 · 서울시립대)_
- **2025.09** : 가천대학교 창의적 종합설계 경진대회 **설계상** _(가천대학교)_
- **2025.09** : 제23회 TOPCIT 성적우수자 **우수상** _(가천대학교 소프트웨어융합교육원)_
- **2025.08** : SK AI Dream Camp 고급 과정 **상위 10% 우수 학습자** _(SK AI Dream Camp)_
- **2025.05** : 춘계학술대회 **우수발표 논문상** _(한국인공지능융합기술학회)_
- **2021.07** : AI-SW 페스티벌 **우수상** _(가천대학교)_



<!--**📫 Contact**
- Email: **lucentejjh@gmail.com*-->

<!--
**🚀 Featured Projects**

- **VIBR** | `2025.12 - 2026.02` | [GitHub](https://github.com/boostcampwm2025/web17-Busy) · [Service](https://vibr.site/)  
  링크 공유로 끝나던 음악 추천을 피드 기반 흐름으로 확장한 소셜 뮤직 큐레이션 플랫폼  
  - **역할**
    - FE·BE 전반에서 검색, 재생, 로그인, 좋아요·댓글·팔로우의 핵심 사용자 흐름 구현
    - Google OAuth2 로그인, 전역 플레이어, 데이터 무결성 및 DX 안정화
    - 추천 고도화를 위한 사용자 로그 수집 파이프라인 설계 및 Redis Stream 기반 비동기 적재 구조 구성
  - **성과**
    - 핵심 사용자 플로우 완성
    - 장애성 버그 및 무결성 이슈 개선

- **RushWash** | `2025.03 - 2025.06` | [GitHub](https://github.com/Jae-Hyuk-Jang/RushWash)  
  의류 이미지로 얼룩·세탁기호를 인식해 맞춤 세탁 지침을 추천하는 경량 멀티모달 AI  
  - **역할**
    - AI 모델, 추론 API, BE-FE-AI 연동 파이프라인 설계
    - 데이터 기준 재정의 및 3,300장 재라벨링 주도
    - 품질 표준화와 실험 재현성 확보
  - **성과**
    - 내부 검증 Top-1 90%, 독립 검증 Top-1 74.1%
    - 한국인공지능융합기술학회 우수발표 논문상 수상

- **Omni Card** | `2025.07 - 2025.09` | [GitHub](https://github.com/2025-Gachon-capstone/Omni-GNN/tree/jhyuk)  
  실구매 시퀀스를 학습해 성과 기반 혜택을 연결하는 카드 마케팅 플랫폼  
  - **역할**
    - XGBoost Minimal-5 vs +Embedding 설계·검증
    - Neo4j 기반 GNN 구조 설계
    - 그래프·시퀀스·트리 모델 혼합 파이프라인 구성 참여
  - **성과**
    - 내부 오프라인 검증 OOF F1≈0.751, 팀 테스트 F1≈0.705
    - 가천대학교 창의적 종합설계 경진대회 설계상 수상
-->

**🚀 Featured Projects**

| 프로젝트명 | 기간 | 기술 스택 | 링크 |
|---|---|---|---|
| **VIBR - with AI** | `2026.07 ~ now` | Next.js, React, TypeScript, NestJS, TypeORM, Zustand, MySQL, Redis | [**Repo**](https://github.com/Jae-Hyuk-Jang/web17-boostcamp-vibr) |
| **VIBR** | `2025.12 - now` | Next.js, React, TypeScript, NestJS, TypeORM, Zustand, MySQL, Redis | [**Repo**](https://github.com/boostcampwm2025/web17-Busy) · [**Service**](https://vibr.site/) |
| **RushWash** | `2025.03 - 2025.06` | Python, FastAPI, MySQL, YOLOv8, LabelImg | [**Repo**](https://github.com/Jae-Hyuk-Jang/RushWash) |
| **Omni Card** | `2025.07 - 2025.09` | Python, PyTorch, PyG, Neo4j, XGBoost | [**Repo**](https://github.com/2025-Gachon-capstone/Omni-GNN/tree/jhyuk) |

